...
if not TOKEN:
    ...
    # В Vercel токен будет задан через переменные окружения
...
# ... (весь код между этой строкой и обработчиками вебхука остается без изменений) ...
# Главное меню
def main_menu():
...
# Установка вебхука - изменяем для работы на Vercel
@app.route('/set_webhook', methods=['GET', 'POST'])
def set_webhook():
    try:
        # Динамически определяем URL текущего приложения
        # На Vercel мы можем использовать заголовок Host для построения URL
        host = request.host
        # Если запрос идет через HTTPS (что должно быть на Vercel)
...
# Для Vercel нам не нужен запуск через app.run, т.к. Vercel сам запустит приложение
# if __name__ == '__main__':
#     app.run(host='0.0.0.0', port=5000)

# Экспортируем приложение для Vercel
# Vercel требует, чтобы WSGI-приложение было доступно как переменная `app` или `handler`
# У нас уже есть `app`, поэтому ничего дополнительно делать не нужно